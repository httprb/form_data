...
class FormDataFileTest < Minitest::Test
  ...
  def fixture_path
    fixture("the-http-gem.info")
  end

  def fixture_content
    fixture_path.read(mode: "rb")
  end
  ...
  def each_source
    ...
    yield HTTP::FormData::File.new(fixture_path.to_s), "string_path", content
    yield HTTP::FormData::File.new(fixture_path), "pathname", content

    file = fixture_path.open("rb")
    begin
      yield HTTP::FormData::File.new(file), "file", content
    ensure
      file.close
    end
  ...
  # Uses an IO that responds to :path but not :to_path, so
  # ::File.basename(io) would raise TypeError while ::File.basename(io.path) works.
